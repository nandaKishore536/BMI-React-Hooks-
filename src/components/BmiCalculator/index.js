...
const BmiCalculator = () => {
  const x = JSON.parse(localStorage.getItem('height'))
  const y = JSON.parse(localStorage.getItem('weight'))
  const [height, setHeight] = useState(x !== null ? x : 170)
  const [weight, setWeight] = useState(y !== null ? y : 60)
